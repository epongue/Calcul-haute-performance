...
int main(int argc, char **argv) {
  ...
  flop_count += 1;                   // 1 div (pour ceil, approximation)
  ...
  for (int k = 0; k < K; ++k) {
    for (int n = 0; n < Np; ++n) {
      ...
      flop_count += 7; // 3 mult (x*x, y*y, z*z) + 2 add + 1 div + 1 exp
    }
  }
      ...
      for (int k = 0; k < K; ++k) {
        ...
        for (int f = 0; f < NFacesTet; f++) {
          ...
          for (int nf = f * Nfp; nf < (f + 1) * Nfp; nf++) {
            ...
            if (n2 >= 0) { // ... if there is a neighbor element ...
              ...
              flop_count += 11; // 2 mult (rhoP * cP, rho * c) + 2 div + 1 add + 1 div + 2 sub + 1 mult + 1 sub + 1 mult
              s_u_flux[nf] = nx * c / (rhoP * cP + rho * c) * ((pP - pM) - rhoP * cP * (nMdotuP - nMdotuM));
              flop_count += 8; // 4 mult + 1 div + 1 add + 2 sub
              ...
              flop_count += 3; // 1 mult + 1 div + 1 mult

              // Penalty terms for boundary of the domain
              s_p_flux[nf] = -c / (1. / (rhoP * cP) + 1. / (rho * c)) * tmp;
              flop_count += 6; // 1 mult + 2 div + 1 add + 1 div + 1 mult
              s_u_flux[nf] = nx * c / (rhoP * cP + rho * c) * tmp;
              flop_count += 5; // 2 mult + 1 div + 1 add + 1 mult
        ...
        for (int n = 0; n < Np; ++n) {
          ...
          for (int m = 0; m < Np; ++m) {
            ...
            dwdr += Dr * s_w[m];
            flop_count += 4; // 4 mult
            flop_count += 3; // 3 add (accumulation)
      ...
      for (int k = 0; k < K; ++k) {
        for (int n = 0; n < Np; ++n) {
          for (int iField = 0; iField < Nfields; ++iField) {
            ...
            flop_count += 3; // 2 mult + 1 add
            _valQ[id] = _valQ[id] + b * _resQ[id];
            flop_count += 2; // 1 mult + 1 add
          }
        }
      }
    }
  ...
  flop_count += 1; // 1 mult (Nsteps * dt)

  // Afficher le nombre total de FLOPs
  double nflops = 120*K*Nsteps*((N+1)*(N+2)*(N+3)/6)^2;
  cout << "Total FLOPs: " << flop_count << endl;
  cout << "total FLOPs THheorique " << nflops << endl;
  return 0;
}